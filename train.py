...
def train():
    ...
    model = CorefModel(config, dataloader.dep_tag2id)
    ...
    step = 0
    warmup_linear(bert_optimizer, config, step + 1, num_warmup_steps)
    ...
    for epoch in range(config['num_epochs']):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            subtoken_map, graph, arg_ids, arg_mask, predicates = [b.to(device) for b in batch[1:]]
            ...
            if step > 0 and step % eval_frequency == 0:
                coref_p, coref_r, coref_f = evaluate(model, eval_dataloader, config['eval_path'], config['singleton'],
                                                     device)
                ...
                if coref_f > best_dev_f1:
                    ...
                    test_coref_p, test_coref_r, test_coref_f = \
                        evaluate(model, test_dataloader, config['test_path'], config['singleton'], device)
...
def evaluate(model, eval_dataloader, data_path, singleton, device):
    with open(data_path) as f:
        examples = [json.loads(jsonline) for jsonline in f.readlines()]

    model.eval()
    coref_evaluator = metrics.CorefEvaluator(singleton=singleton)
    with torch.no_grad():
        for i, (batch, example) in enumerate(zip(eval_dataloader, examples)):
            ...
            predictions, loss = model(input_ids, input_mask, text_len, speaker_ids, genre, gold_starts, gold_ends,
                                      cluster_ids, sentence_map, subtoken_map, graph, arg_ids, arg_mask, predicates)
            (candidate_cluster_ids, candidate_starts, candidate_ends, candidate_mention_scores, top_span_starts,
             top_span_ends, top_antecedents, top_antecedent_scores, top_span_mention_scores) = \
