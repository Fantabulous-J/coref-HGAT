...
def evaluate(model, eval_dataloader, data_path, conll_path, prediction_path, singleton, device):
    ...
    with torch.no_grad():
        for i, (batch, example) in enumerate(zip(eval_dataloader, examples)):
            ...
            graph, arg_ids, arg_mask, predicates = [b.to(device) for b in batch[1:]]
...
if __name__ == '__main__':
    ...
    test_coref_p, test_coref_r, test_coref_f, test_conll_results = \
        evaluate(model, test_dataloader, config['test_path'], config['conll_test_path'], test_prediction_file,
                 config['singleton'], device)
