...
# srl_labels = ['AM-EXT', 'R-AM-CAU', 'AM-LOC', 'AM-CAU', 'AM-NEG', 'C-AM-CAU', 'A3', 'C-AM-MNR', 'R-AM-ADV', 'C-A4',
#               'AM-ADV', 'A1', 'AM-MNR', 'R-A3', 'C-AM-TMP', 'R-AM-DIR', 'R-AM-MNR', 'A5', 'C-A2', 'C-A1', 'R-AA',
#               'R-A1', 'C-AM-PNC', 'R-A2', 'AA', 'AM-TMP', 'AM-MOD', 'C-V', 'C-AM-NEG', 'AM-REC', 'R-A4', 'R-AM-EXT',
#               'A2', 'AM', 'C-AM-DIS', 'C-AM-DIR', 'AM-TM', 'C-AM-ADV', 'C-A0', 'R-A0', 'AM-PNC', 'A4', 'V', 'A0',
#               'C-A5', 'AM-DIS', 'C-AM-EXT', 'C-AM-LOC', 'R-AM-LOC', 'AM-PRD', 'R-AM-PNC', 'AM-DIR', 'C-A3', 'R-AM-TMP']
...
def read_path_to_root(data_path):
    with open(data_path, "r") as f:
        examples = [json.loads(jsonline) for jsonline in f.readlines()]

    path_to_root_examples = []
    root_id_examples = []
    ...
    for example in examples:
        ...
        for sent_feature in depparse_features:
            ...
            for feature in sent_feature:
                temp = [feature['id'] - 1]
                head_id = feature['head_id']
                if head_id == 0 and root_id is None:
                    root_id = feature['id'] - 1
                while head_id != 0:
                    temp.append(head_id - 1)
                    head_id = sent_feature[head_id - word_offset - 1]['head_id']
                path_to_root.append(temp)
            assert root_id is not None
            root_ids.append(root_id)
            word_offset += len(sent_feature)
            paths.append(path_to_root)
        assert len(root_ids) == len(depparse_features), (len(root_ids), len(depparse_features),
                                                         [feature for sent_feature in depparse_features for feature in
                                                          sent_feature if feature['head_id'] == 0])
        path_to_root_examples.append(paths)
        root_id_examples.append(root_ids)

    return path_to_root_examples, root_id_examples
...
if __name__ == '__main__':
    ...
    #             f.write(tag + " " + str(id) + "\n")
    #     read_path_to_root(data_path="conll_data/test.english.depparse.gold.jsonlines")

    with open("conll_data/train.english.depparse.bert-biaffine.jsonlines", "r") as f:
        examples = [json.loads(jsonline) for jsonline in f.readlines()]

    with open("conll_data/dev.english.depparse.bert-biaffine.jsonlines", "r") as f:
        examples += [json.loads(jsonline) for jsonline in f.readlines()]

    with open("conll_data/test.english.depparse.bert-biaffine.jsonlines", "r") as f:
        examples += [json.loads(jsonline) for jsonline in f.readlines()]
    ...
    label2dict = {"<pad>": 0, "cyclic": 1}
    for i, label in enumerate(tag_set):
        label2dict[label] = i + 2

    with open("conll_data/dep_bert-biaffine_vocab.txt", "w") as f:
