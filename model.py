...
from modules import MultiHeadGATLayer, TGATLayer, TAGATLayer, APGATLayer, PAGATLayer, ATGATLayer
import util
import logging
import coref_cpp
...
class CorefModel(nn.Module, ABC):
    def __init__(self, config, dep_tag2id):
        ...
        self.dep_edge_embeddings = nn.Embedding(len(dep_tag2id), config['dep_hidden_size'])
        nn.init.trunc_normal_(self.dep_edge_embeddings.weight.data, std=0.02)

        self.srl_edge_embeddings = nn.Embedding(len(util.srl_labels), config['srl_hidden_size'])
        nn.init.trunc_normal_(self.srl_edge_embeddings.weight.data, std=0.02)
        ...
        feature_size = 4 * config['feature_size'] if self.config['use_metadata'] else 3 * self.config['feature_size']
        ...
        self.tt_gat_layer = MultiHeadGATLayer(TGATLayer, config['hidden_size'], config['hidden_size'],
                                              config['dep_hidden_size'], config['num_heads'], config)
        self.ta_gat_layer = MultiHeadGATLayer(TAGATLayer, config['hidden_size'], config['hidden_size'],
                                              config['feature_size'], config['num_heads'], config)
        self.ap_gat_layer = MultiHeadGATLayer(APGATLayer, config['hidden_size'], config['hidden_size'],
                                              config['srl_hidden_size'], config['num_heads'], config)
        self.pa_gat_layer = MultiHeadGATLayer(PAGATLayer, config['hidden_size'], config['hidden_size'],
                                              config['srl_hidden_size'], config['num_heads'], config)
        self.at_gat_layer = MultiHeadGATLayer(ATGATLayer, config['hidden_size'], config['hidden_size'],
                                              config['feature_size'], config['num_heads'], config)
        ...
        self.fusion_gate = self.ffnn(input_size=config['hidden_size'] * 2,
                                     ...
                                     dropout=None)
        self.linear_dep = self.ffnn(input_size=2 * config['hidden_size'],
                                    num_hidden_layers=0,
                                    hidden_size=-1,
                                    output_size=1,
                                    dropout=None)
        self.linear_srl = self.ffnn(input_size=2 * config['hidden_size'],
                                    num_hidden_layers=0,
                                    hidden_size=-1,
                                    output_size=1,
                                    dropout=None)
    ...
    def forward(self, input_ids, input_mask, text_len, speaker_ids, genre, gold_starts, gold_ends, cluster_ids,
        ...
        """
            ...
            Args:
                ...
                speaker_ids: [num_segments, max_segment_len]
                genre: [1]
                ...
                arg_ids: [num_arguments, max_arg_len]
                arg_mask: [num_arguments, max_arg_len]
        ...
        # token -> token edge embeddings
        tt_edge_id = graph.filter_edges(lambda edges: edges.data["dtype"] == 0)
        tt_edge = graph.edges[tt_edge_id].data['dep_link']
        graph.edges[tt_edge_id].data['dep_emb'] = self.dep_edge_embeddings(tt_edge)

        # argument and predicate nodes
        if arg_ids.size()[0] != 0:
            # [num_arguments, max_arg_len, embed_size]
            arg_embeddings = token_embeddings[arg_ids] * arg_mask.unsqueeze(-1)
            # [num_arguments, embed_size]
            arg_embeddings = torch.sum(arg_embeddings, dim=1) / torch.sum(arg_mask, dim=1, keepdim=True)
            predicate_embeddings = token_embeddings[predicates]

            # argument -> predicate edge embeddings
            ap_edge_id = graph.filter_edges(lambda edges: edges.data['dtype'] == 2)
            ap_edge = graph.edges[ap_edge_id].data['srl_link']
            graph.edges[ap_edge_id].data['srl_emb'] = self.srl_edge_embeddings(ap_edge)

        # two way (dep + srl) (1) 78.9 (both golden labels)
        # 1. Token -> Token (dependency parsing)
        # 2. Token -> Argument => Argument -> Predicate => Predicate -> Argument => Argument -> Token
        # 3. Token -> Sentence -> Speaker -> Sentence -> Token
        token_embeddings_dep = token_embeddings
        for i in range(self.config['num_iterations']):
            token_embeddings_dep = self.tt_gat_layer(graph, token_embeddings_dep, token_embeddings_dep)

        token_embeddings_srl = token_embeddings
        if arg_ids.size()[0] != 0:
            for i in range(self.config['num_iterations']):
                arg_embeddings = self.ta_gat_layer(graph, arg_embeddings, token_embeddings_srl)
                predicate_embeddings = self.ap_gat_layer(graph, predicate_embeddings, arg_embeddings)
                arg_embeddings = self.pa_gat_layer(graph, arg_embeddings, predicate_embeddings)
                token_embeddings_srl = self.at_gat_layer(graph, token_embeddings_srl, arg_embeddings)

        weight_dep = torch.sigmoid(self.linear_dep(torch.cat([token_embeddings, token_embeddings_dep], dim=-1)))
        weight_srl = torch.sigmoid(self.linear_srl(torch.cat([token_embeddings, token_embeddings_srl], dim=-1)))
        # [num_tokens, 1, 2]
        weight = torch.softmax(torch.cat([weight_srl, weight_dep], dim=-1), dim=-1).unsqueeze(1)
        # [num_tokens, 2, emb_size]
        enhanced_embeddings = torch.cat([token_embeddings_srl.unsqueeze(1), token_embeddings_dep.unsqueeze(1)], dim=1)
        enhanced_embeddings = torch.bmm(weight, enhanced_embeddings).squeeze(1)

        f = torch.sigmoid(self.fusion_gate(torch.cat([token_embeddings, enhanced_embeddings], dim=-1)))
        token_embeddings = f * token_embeddings + (1 - f) * enhanced_embeddings
    ...
    def get_masked_sentence_word_scores(self, sentence_embeddings, top_span_sentences, top_antecedent_sentences):
        num_sentences = sentence_embeddings.size()[0]
        k = top_span_sentences.size()[0]
        c = top_antecedent_sentences.size()[1]
        sent_range = torch.arange(0, num_sentences, device=sentence_embeddings.device).unsqueeze(0).expand(k * c, -1)

        top_span_sentences = top_span_sentences.unsqueeze(1).expand([-1, c]).reshape([-1])
        top_antecedent_sentences = top_antecedent_sentences.reshape([-1])
        # [k * c, num_sentences]
        sentence_mask = torch.logical_and(
            sent_range >= torch.minimum(top_antecedent_sentences.unsqueeze(1), top_span_sentences.unsqueeze(1)),
            sent_range <= torch.maximum(top_antecedent_sentences.unsqueeze(1), top_span_sentences.unsqueeze(1)),
        )
        # [num_sentences]
        sentence_atten = self.sentence_word_attn_layer(sentence_embeddings).squeeze(1)
        # [k * c, num_sentences]
        sentence_word_attn = torch.softmax((torch.log(sentence_mask.float())) + sentence_atten.unsqueeze(0), dim=-1)

        return sentence_word_attn
